/**
 * Integration tests for series cron: runSeriesRollCron.
 *
 * Requires local Supabase running (SUPABASE_TEST_* env vars).
 * Unit tests for shouldPromote + nextOccurrencesToMaterialize live in series-cron.test.ts.
 */

import { describe, it, expect, beforeAll, afterAll, vi } from "vitest";
import { createClient } from "@supabase/supabase-js";
import { runSeriesRollCron } from "./series-cron";
 ...
 */
const EMAIL_PREFIX = "test-series-";
...
beforeAll(async () => {
  ...
  [nearUserId, blockerUserId, debtorUserId] = await Promise.all([
    createUser(`${EMAIL_PREFIX}near-${ts}@example.invalid`, NEAR_LAT, NEAR_LNG),
    createUser(
      `${EMAIL_PREFIX}blocker-${ts}@example.invalid`,
      NEAR_LAT,
      NEAR_LNG,
    ),
    createUser(
...
describe("runSeriesRollCron — promote", () => {
  it("promotes a time-only pending booking now inside the horizon, leaves a distance-manual one pending", async () => {
    ...
    // Stubbed so the run never reaches Resend; the assertions below are about
    // which bookings the cron considers confirmed.
    const sendConfirmation = vi.fn(async () => {});

    const result = await runSeriesRollCron({
      serviceClient,
      now,
      sendConfirmation,
    });
    ...
    // A promoted booking is a confirmed transition, so it gets its email; the
    // two that stayed pending must not.
    expect(sendConfirmation).toHaveBeenCalledWith(autoId);
    expect(sendConfirmation).not.toHaveBeenCalledWith(manualId);
    expect(sendConfirmation).not.toHaveBeenCalledWith(debtorId);
