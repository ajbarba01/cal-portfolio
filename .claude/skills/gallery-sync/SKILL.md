...
- `public/gallery/` outputs are content-hashed (cache-busting); `public/bg/` outputs keep stable basenames (they're referenced by path in code).
- Commit the re-encoded `public/` outputs and `image-placeholders.json` in the same commit.
- Don't hand-edit `image-placeholders.json` or `public/gallery|bg` outputs — regenerate via the script.
