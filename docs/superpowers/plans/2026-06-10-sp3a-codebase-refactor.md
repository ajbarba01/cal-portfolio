...
- **2026-06-10 · RESOLVED (was BLOCKING) · feature barrels poison the client bundle; `next build` fails.**
  - **Resolution (maintainer approved Option A).** Commit `fa8e25a`: added client-safe entry points `src/features/booking/index.client.ts` + `src/features/payments/index.client.ts` (client surface = components/hooks/types/`"use server"` actions; EXCLUDES non-action server-only modules — `loadBookingFormData`, `createSupabaseBookingRepository`, `StripeGateway`, etc.); repointed 16 `"use client"` files to the client entries; allowed `index.client.ts` as a 2nd boundary entry point in `eslint.config.mjs`; added `docs/adr/0002-client-server-entry-points.md` refining ADR-0001. `accounts`/`notifications` needed no client entry (accounts barrel only re-exports actions/types/client-comps; its only taint was the transitive `MeetGreetScheduler`→booking import, fixed by repointing that component). Verified: `npm run build` exit 0, 0 server-only errors, 32/32 static pages; typecheck + lint clean; named booking suites green. **ADR-number consequence:** ADR-0002 is now this entry-point split, so the plan's Task 7 ADR (action-mutation) becomes **0003** and Task 8 (notifier) becomes **0004**.
  - _Original report:_
...
- **2026-06-10 · Minor · Task 6 hook location deviation + caught regression.** (1) The plan put the extracted hooks in `src/features/booking/_components` and `src/features/admin/_components`, but the three god-components live under `src/app/.../_components`. With boundaries enforced, an `src/app` file importing a deep `src/features/*` path is a violation, and exposing route-only state through a feature entry point would over-expose it. So each hook was co-located NEXT TO its component (`use-service-booking.ts`, `use-edit-booking.ts`, `use-admin-create-booking.ts` in the same app `_components` folder, relative import). Behaviorally identical, cleaner ownership. (2) Review caught a behavior regression in the first Task 6 pass: `admin-create`'s `onSelectionChange` was changed from `useCallback([mode])` to a plain function. `Scheduler` subscribes to `onSelectionChange` in a `useEffect` dep array (`scheduler.tsx:61`), so an unstable identity would re-fire the effect every render (spurious setState + `requestQuote()`, render-loop risk). Fixed in `99040d4` (restored `useCallback([mode])` verbatim). Verified all three hooks now match their originals' hook-primitive counts exactly; build + integration tests green. Manual-QA eyeball items for the DoD review: deferred-auth returnTo rehydration (service-booking), edit initial-slot pre-selection, admin force-confirm checkbox.
