...
- **`src/features/inquiries/inquiry-schema.ts` is not an admin surface, despite landing in this group.** The extractor's group rules are first-match-wins and `^src/features/inquiries/` matches before the schema rule, so its validation messages (`Name is required`, `Enter a valid email`, `Phone is required`, `Message is required`) were routed here instead of to `E-validation`. They render inline on the **public** `/contact` page through `contact-form.tsx`. Audit them under Group E's rules, not Group G's: they belong to the `X is required` family, and the set-consistency rule applies — a flag has to argue the whole set is wrong, not one member of it.
