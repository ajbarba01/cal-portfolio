...
- [ ] **Step 1: Add "Doc lifecycle" section to WORKFLOW.md** (verbatim, before "## Working within a task"):
...
- [ ] **Step 2: Create `scripts/check-doc-links.mjs`:**
...
- [ ] **Step 1: Confirm absorption.** Every current DEV_NOTES item must appear in the [findings register](../specs/2026-06-10-audit-findings.md) (findings, triage decisions, or Cal/ops list) or the roadmap. Diff the lists; anything missed gets added to the register **first** (separate commit to the register).

- [ ] **Step 2: Replace DEV_NOTES.md content entirely with:**
...
- [ ] **Step 1: Confirm OTHER.md deletion with the maintainer** (it is another project's instruction file used as a style reference; the original lives in that project). One-line ask; on yes: `git rm OTHER.md`. On no: move it to `docs/superpowers/specs/reference-other-project-style.md` instead.
...
- [ ] **Step 1: Record baselines:** `wc -w docs/WORKFLOW.md docs/ROLES.md docs/ROUTING.md docs/ENGINEERING.md docs/CODE_STYLE.md docs/FRONTEND.md`

- [ ] **Step 2: Compress one doc at a time** using the Shared-reference ruleset. After each doc: re-read the diff hunk-by-hunk asking "did a rule, value, or link disappear?" — restore anything lost. Bump its `_Last reviewed:` to today.

- [ ] **Step 3: Verify per doc, commit per doc**
...
- [ ] **Step 1: Create `.claude/caveman.md`:**
...
- [ ] **Step 3: Update CLAUDE.md:** remove the "If `/caveman` is available, invoke `/caveman full`…" line (hook supersedes it); note instead: "Caveman mode auto-injected via `.claude/settings.json` SessionStart hook." Compress the remainder per ruleset.

- [ ] **Step 4: Compress AGENTS.md** per ruleset. The doc-nav table, constitution bullets, and layout line keep all content; prose around them tersens. AGENTS.md communication-mode rule stays (it is the model-neutral source; the hook is Claude's enforcement of it).
...
- [ ] **Step 4: Request cross review.** Run `/code-review` on the cumulative SP1 diff (docs-only review: fact loss, broken cross-references, portability leaks). Resolve criticals via the Handoff log below.
...
_(empty — implementer appends escalations here, commits, and stops per WORKFLOW.md)_
