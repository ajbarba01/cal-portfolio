...
Sitewide application of SP3 primitives. Rule: **every user action produces visible feedback** — hover/click states, toasts, inline, redirects; nothing silent. Redirect-back after auth, post-booking redirect, text wrap/overflow states, empty states, toast redesign application, popup restyle application, own-bookings muted-clay, calendar polish, breakpoint-transition correctness, mobile parity (mobile as intentional as desktop). Focus: as far from generic "vibe-coded" aesthetics as possible.

**Planning session 2026-06-11 (senior-designer, Fable 5):** Alex's SP6 reminder notes + the Fable 5 audit ask are fully integrated into the [SP6 spec](2026-06-11-sp6-cohesion-design.md) (decisions table + findings U14–U31) and moved out of this doc. Audit pass ran (live screenshots all public pages, 1440/390 + static); 4 mockups signed off one at a time in [`mockups/sp6/`](../mockups/sp6/NOTES.md). **Sequencing decision:** SP6 runs **before** the interleaved P2–P4/debt features (reversing the original slot) — those feature specs must carry an "SP6 conventions" checklist and get a small-delta re-sweep when they land. DEV_NOTES "Now" inbox fully triaged into the register (U24–U31 + feature-lane Kiche item).
...
Feature-sized DEV_NOTES items are **not** findings: booking-mutation P2-P4 (client self-edit, cancel bookings + reason + refund semantics, reschedule generalization, recurring rework), account inquiries tab, Kiche per-booking discount (added 2026-06-11). Originally slotted after SP4 and before SP6; **re-sequenced 2026-06-11 (maintainer):** SP6 runs first — these features inherit SP6 conventions at build time (their specs carry an "SP6 conventions" checklist) and get a small-delta re-sweep, not a second full pass.
