...
REMINDER FROM ALEX FOR SP6 (integrate into planning): add visible page loading and ensure all content widths are cohesive and make sense throughout pages. visual mockups should be made extensively for this SP. also prefer lucide icons over emojis. also add confirm dialogs wherever necessary (like marking balance as settled in admin). also im thinking maybe booking options should be a popup (think through with me). more points for cohesion: similar component types should have the same styling (like any type of form should be similar in bg color and input box color and whatnot). also as a general rule of thumb for this pass, compressing a pages width from desktop to mobile should have the least amount of reorganizations as possible (but of course should look neat at any time). I do want Fable 5 to run another audit pass for this SP with its attention to detail so raise this to the user when reading if you are not Fable 5. a few examples of the types of things I want it to catch: the spacing of the 3 trust points on the home page, and the lack of styling or contrast on the contact page. I also want this pass to focus on making this site as far removed from the vibe coded style that we see all the time now. also another note some of the dropdowns on the side are just a little weird in terms of spacing/styling. also nav bar needs a little attention.
