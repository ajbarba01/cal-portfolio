...
Surfaced by the SP3a fresh-session `/code-review`. All are **pre-existing duplication/structure** that SP3a relocated verbatim (not regressions); consolidating them is new abstraction work needing its own tests, so deferred rather than expanding SP3a at DoD.

| ID  | Sev | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Target                  |
| --- | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------- |
| A13 | M   | `use-service-booking` / `use-admin-create-booking` / `use-edit-booking` are ~90% duplicated (date helpers, `buildCapabilities`, `busyRanges`, `schedulerData`, stay/derivation, latest-ref debounce, `onSelectionChange`, `@`-cell parsing, step labels) — only deltas are action calls + auth/forceConfirm gating. Generalize into a shared `useBookingScheduler` primitive parameterized by `{preview, submit, gating}`. Risk if left: three hooks kept in lockstep by hand; a fix to selection→time derivation / 400 ms debounce / cell parsing must be applied 2–3×. | SP3b                    |
| A14 | M   | `SettingsRow→BookingRuleSettings` mapping + `passesGuards`/`getOpenWindows`/`fitsWindow` slot-validation block duplicated 4× across cores (`create-core`, `edit-core` ×2 incl. `previewEditCore`, `reschedule-core`) despite `booking-service-shared`'s "≥2 cores" charter; `previewEditCore` is a near-verbatim clone of `editBookingCore`. Extract `toRuleSettings()` + `validateSlot()` into shared. Risk if left: edit vs preview can drift, breaking the WYSIWYG guarantee the preview twin exists for.                                                             | SP3b / booking-mutation |
...
| A16 | m   | `create-core` re-runs `createBookingInputSchema.parse(rawInput)` that `computeBookingArtifacts` already ran (shared discards its parsed `input`, forcing the re-parse to recover `userId`/`startsAt`/`endsAt`). Surface the already-parsed input on `BookingQuoteArtifacts` (e.g. `artifacts.input`) to remove the redundant validation + divergence risk.                                                                                                                                                                                                               | SP3b / booking-mutation |
