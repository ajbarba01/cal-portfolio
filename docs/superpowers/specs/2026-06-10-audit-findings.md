...
| PAY6 | m   | Unhandled webhook events: `charge.dispute.*`. Add to event map or consciously ignore with comment. (`payment_intent.canceled` handled in the payments-flow slice → row `failed`.)                                                                                                            |
...
- PAY1, PAY2, PAY4 + the `payment_intent.canceled` half of PAY6 resolved by the payments-flow slice, 2026-06-11: PaymentElement mounted in the account-bookings dialog; test-mode setup documented in `.env.example`; open-intent reuse + booking-scoped idempotency (cancel-and-recreate on stale, key rotated); `payment_intent.canceled` → row `failed`, `bookings.status` untouched. PAY3/PAY5/`charge.dispute.*`/PAY7 remain for the hardening slice (SP4b). Live `verify` (real `stripe listen` money-movement) outstanding — pending Stripe CLI install.
