...
- **`form_responses`** — **entity-scoped** reusable intake profiles. `id` · `client_id` · `form_key` ('owner' \| 'home' \| 'pet'; legacy 'emergency' retained, superseded by 'owner') · `pet_id` (nullable FK→pets `on delete cascade`; set only for pet-scoped 'pet' rows) · `booking_id` (nullable) · `data` (jsonb) · `submitted_at` (bumped on every submit **and** on a "Confirm up to date" action — drives staleness). Uniqueness is scope-aware: `(client_id, form_key, coalesce(pet_id, nil-uuid))`, so account profiles are one-per-client and pet profiles one-per-pet. The `scope` ('account' \| 'pet') + Zod schema per `form_key` live in `src/features/accounts/form-registry.ts` (code, validated at the edge — YAGNI, [ENGINEERING.md](ENGINEERING.md) #9). Owner = identity/pronouns/backup/2nd–3rd owner/emergency contacts; Home = address/access/alarms/wifi/breaker/sleeping/home-care/house-rules/guest-policy; Pet = medical/behavior/feeding/exercise. Bookings **require and confirm** these rather than re-collecting — see Booking requirement gate.
...
Cal's six service-shaped intake forms are **normalized into three reusable, account-based profiles** rather than re-collected per booking: **Owner** (account), **Home** (account, one-per-account today; schema allows multi later — `form_responses` is already pet-scopeable), and **Pet** (per-pet). Each is a `form_responses` row keyed by `form_key` + scope (see Data model). `FORMS.md` (repo root) is Cal's raw, owner-written source; it is requirements to normalize, not a schema.

**Requirement manifest (code, not schema).** `REQUIRED_PROFILES` in `src/features/booking/required-profiles.ts` maps `pricing_type` → required profiles: `house_sitting` → [owner, home, pet]; `walk`/`check_in`/`training` → [owner, pet]; `meet_greet` → [owner]. Kept in code so Cal can't misconfigure required forms; `services.form_key` is legacy and no longer consulted by the gate.
...
_Last reviewed: 2026-06-16_ (intake forms normalized into reusable Owner/Home/Pet profiles; `form_responses` entity-scoped via `pet_id` + scope-aware uniqueness; `pets` identity columns; append-only `authorizations` e-sign; code `REQUIRED_PROFILES` manifest + hard-block requirement gate replacing the single `form_key` gate)
