...
Cal can create a client record before the person signs up — to approve them, fill forms, and book on their behalf — via an **unclaimed shadow account**. `createUnclaimedClient` (admin-gated, service role) mints a real `auth.users` row with **no password**; the existing `handle_new_user` trigger creates the `profiles` row, which the action flags `unclaimed = true`. Every existing on-behalf flow (booking, forms, approval dropdown, RLS, exclusion constraint) is reused unchanged. Automated notifications are **suppressed while unclaimed** (`shouldNotify` predicate at each dispatch boundary — confirmation send + reminder cron); Cal handles comms manually. To migrate an offline client, Cal generates a one-time **claim link** (Supabase `invite` `generateLink`, delivered by copy — never auto-emailed), which lands the client at `/claim` (via `/auth/callback?next=/claim`) to set a password; that clears `unclaimed` and stamps `claimed_at`, handing routing back to the `onboarding_status` gate. Email collision on create maps to a friendly "already exists" pointing at the existing client. Design: [spec](superpowers/specs/2026-06-24-admin-precreate-client-design.md).
