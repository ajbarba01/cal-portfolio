...
gained a courtesy carve-out — a "please" or "sorry" is filler in a routine
...
| 7   | Client location is too far (`${miles}` mi). Hard cutoff is `${cutoff}` mi.      | Client is `${miles}` mi away — beyond the `${cutoff}` mi cutoff. | Merged two clipped sentences, matching the phrasing the sibling warning in the same file already uses. Both interpolated values preserved. |
...
- #7's refusal string is consumed by both the client's own booking flow and
  the admin's book-on-behalf flow, so the subject has to stay "Client" —
  "your location" would be wrong when an admin reads it.
- #9 says "5-digit ZIP code" while its regex also accepts ZIP+4.

## Sample extension — definitional copy (added after round 1)

Round 1 exposed a genre gap: every string in Set A is a label, an empty
state, an error, or a confirmation. None of them is **definitional copy** —
...
| --- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------- |
| 15  | `Holiday & peak-date rate — a surcharge that applies on major holidays and other high-demand dates.` | `src/features/pricing/term-descriptions.ts` | client  |
| 16  | `Long stay — applies once a booking runs longer than 4 nights.`                                      | `src/features/pricing/term-descriptions.ts` | client  |
...
straight to the file — it routes through copy-sync like any other change to
Cal-owned copy.
