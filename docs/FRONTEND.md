...
- **Width scale.** Three `PageContainer` widths: `read` (`max-w-[65ch]`, ~50–75 chars/line body-text target), `narrow` (`max-w-xl` / ~36rem, auth and intake forms), `app` (`max-w-6xl` / ~72rem, tables and admin). The booking flow is the one documented exception: it sets its own wider column, and the pages that host it match that width rather than passing a `PageContainer` width. `SiteHeader` and `SiteFooter` share the same `max-w-6xl` inner container so chrome edges align with app-width content. Marketing section bands are full-bleed background + `max-w-6xl` inner container — the band color bleeds, the content aligns.
...
- **Component kit.** Built on `@base-ui/react` + semantic tokens, in `src/components/ui/`, `src/components/form/` and `src/components/feedback/`: one card primitive (`Surface`), badge, skeleton, form field, select, and the feedback set (ErrorState, EmptyState, toast, ConfirmDialog), among others — [COMPONENT_SYSTEM.md](COMPONENT_SYSTEM.md) owns the registry and the per-primitive contracts; this bullet does not restate it. Button/input/label/calendar are token-aligned; a `brand` button variant uses `--brand` for primary marketing CTAs (`--primary` stays the neutral default). `FormField` enforces `children` XOR `inputProps` at the type level.
...
- **Form-on-card recipe.** Auth/contact/profile forms sit on a `Surface` (titled groups on `FormSection`), which carries `bg-card` + `border` + the one `rounded-card` radius; inputs inside use `bg-background` + `border-input` + clay focus ring (`focus-visible:border-ring focus-visible:ring-ring/50`). The visual contract: inputs recede into the sheet, card lifts as the form container, single brand submit at the bottom-left (or full-width on narrow cards). The composition rules are in [COMPONENT_SYSTEM.md](COMPONENT_SYSTEM.md) "Form recipe".
...
- **Layer 3 (compound `<Scheduler.*>` parts)** — presentational wireframe only; wired to context; logic-free. `<Scheduler>` root, `<Scheduler.MonthGrid>`, `<Scheduler.DayTimeline>` (read/book-mode start picker), `<Scheduler.DayPainter>` (admin availability editor), `<Scheduler.SelectionSummary>`, `<Scheduler.ClearDates>`, `<Scheduler.Legend>`, `<Scheduler.BookingDetailsPanel>`.

**Visual/interaction model.** Cell STATUS is a background fill (available=green, booked=blue, unavailable=neutral — via status tokens above); SELECTION is an OUTLINE overlay (not a fill) composed on top, so a cell can show both at once. Each selected day is its **own** rounded accent box — outlines do **not** merge across adjacent days (a deliberate "keep days separated" call); only the live drag-preview still uses run-boundary math (pure util in `src/features/booking/grid-runs.ts`). Month grid renders as a gapped `table-fixed` (rdp v9 emits a real `<table>`; equal H/V gaps come from `border-spacing`, not flex/grid gaps). In inspect contexts (admin Bookings hub) a booked cell opens `Scheduler.BookingDetailsPanel`; status colors keyed by `Scheduler.Legend`. **Hourly booking** (short-duration services) uses a month→day flow: month picks the DAY, then `Scheduler.DayTimeline` shows that single day as a duration-accurate vertical timeline for picking/typing the start time; multi-day services stay a month date-range. **Admin availability** is a multi-day flow on the same instrument: the month selects one or more days (booked days included — the booking is awareness-only), then `Scheduler.DayPainter` edits intraday walk windows as a drag-to-create / resize / remove vertical timeline at 15-min granularity, with overnight + premium toggles. Creating hours and toggling fan out across the whole selection; carving time back out stays per-day, so a multi-day selection exposes the bulk paths only. Removing window time a booking overlaps fires a cancel-and-refund confirm.
...
- **Footer never flickers position.** The footer is in the persistent shell, so its vertical position follows content height — unknowable until the page lays out. Both loaders (`DelayedPageLoader`, `PageLoader`) reserve a full viewport (`min-h-dvh`), which pushes the footer below the fold for the whole load on either path (the `pending` swap or a route `loading.tsx`), so it's never seen at a wrong spot. On commit `<FooterReveal>` (an `immediate` `<Reveal>` keyed by `pathname`) fades it in at its final position — the reposition happens on the first frame at opacity 0, so a short page eases in instead of snapping.
...
- **Originals** live in gitignored source folders at the repo root:
  `gallery-originals/` (grid photos), `bg-originals/` (hero/background images),
  `references-originals/` (reference pet-face photos), and
