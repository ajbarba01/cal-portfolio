...
- **`--bg-texture`** — swappable site-wide background pattern (currently a procedural wood-grain SVG
  data-URI, theme-aware via `--tex-wood`; aliased from a `--tex-*` texture library in `globals.css`).
  Applied through the `.bg-texture` utility on the desk (`bg-canvas`) and on `html` so the overscroll
  gutter matches. Lives on the desk only — the sheet/reading surface stays clean. Swap the whole site
  pattern by repointing `--bg-texture` at another `--tex-*`; `none` disables.
