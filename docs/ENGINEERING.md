...
Business-agnostic, reusable infra lives in `lib/` and must contain **zero** knowledge of the business domain. Anything that knows about the app's entities or business rules lives in its feature.

- **Why:** `lib/` stays portable and trivially testable; domain churn never leaks into shared infra.
...
- **Why:** swapping or upgrading a vendor is a one-module change, not an app-wide hunt. Directly serves the goal of swapping pieces with minimal effort.
- **Example:** a `features/<domain>/` module exposes `estimate(input)`; the underlying provider can change while every caller stays untouched.

### 5. Pure core logic, isolated from IO

Core domain calculations and any state machine are **pure functions** over typed inputs — no DB calls, no `fetch`, no clock reads inside them. IO happens at the edges and passes data in.

- **Why:** pure logic is unit-testable without mocks, reusable on server or client, and the part most likely to contain real bugs — so it's the part most worth isolating and testing.
- **Example:** a pure `calculate(inputs)` returns a result; the route handler fetches the inputs and calls it.
...
- **Why:** small units are readable, reusable, and independently testable.

### 7. Typed boundaries everywhere

TypeScript `strict`, no `any`. **Validate and parse external data at the edges** — API responses, DB rows, form input, env vars — with a schema (Zod). Shared types live in one place and are imported, never re-declared.

- **Why:** types catch errors at compile time; runtime validation at the boundary means the rest of the app trusts its data.
...
- **Read first, ground claims in inspection.** Inspect the relevant code before proposing changes; no speculation presented as fact.
