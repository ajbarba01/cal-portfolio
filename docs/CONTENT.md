...
`docs/content/voice/cal.md` describes Cal's writing voice — sentence shape, register,
what gets hedged, what gets stated flat — derived from `cal-source.md` by the
`writing-in-voice` skill. It is applied to **non-marketing text** written about Cal in
third person, through the POV rule above; it does not change what person a surface
writes in. The file is regenerated by the skill's `derive`/`update` operations and
hand-tuned only in its `## Overrides` section — never edit `## Traits` or
